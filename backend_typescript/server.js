...
passport.use(new LocalStrategy(async (username, password, done) => {
    const result = await globals_1.userManager.authenticate(username, password);
...
passport.use(new GoogleStrategy({
    ...
    const result = await globals_1.userManager.googleAuthenticate(issuer, profile.id, profile.emails[0].value, profile.displayName);
    if (result instanceof Result_1.Failure) {
        return done(null, false);
    }
    else if (result instanceof Result_1.Success) {
...
passport.deserializeUser(async (id, done) => {
    const result = await globals_1.userManager.getUserById(id);
    if (result instanceof Result_1.Failure) {
